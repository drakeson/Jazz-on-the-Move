...
class LifestyleVC: UIViewController, KSICCoordinatorDelegate, UICollectionViewDelegate, UICollectionViewDataSource {
...
//        present(alert, animated: true, completion: nil)
    ...
    func loadSlider() {
        ...
        homeSlider.getDeals(Constants.lifestyleURL)
    }
    ...
    func loadDeals() {
        dealsVM.itemsCompletionHandler{ [weak self] (status, items, message) in
            guard self != nil else {return}
            if status {
                DispatchQueue.main.async {
                    self!.itemList = items
        ...
        dealsVM.getItems(Constants.FireBase.LIQOUR)
    }
